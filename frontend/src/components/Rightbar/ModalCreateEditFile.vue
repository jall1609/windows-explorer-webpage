      ...
      <div className="bg-white rounded-lg p-8 w-2/3">
        ...
        <div className="my-3">
          <form  action="#" @submit.prevent="handleSubmit">
              <div className="my-3">
                <label htmlFor="file_name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">File Name</label>
                  <input type="text" name="file_name" id="file_name" className="bg-gray-50 border border-gray-300 text-gray-900 rou nded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Tugas Kuliah" v-model="file_name"  />
                  <div v-for="(err, index) in error.file_name" class="mt-2 text-sm text-red-600 dark:text-red-500">
                    <span className="font-medium">{{ err }}</span>
                  </div>
              </div>
              <div className="my-3">
                <label htmlFor="content" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Content</label>
                <textarea id="content" name="content" rows="5" className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"  v-model="content" ></textarea>
                  <div v-for="(err, index) in error.content" class="mt-2 text-sm text-red-600 dark:text-red-500">
                    <span className="font-medium">{{ err }}</span>
                  </div>
              </div>
